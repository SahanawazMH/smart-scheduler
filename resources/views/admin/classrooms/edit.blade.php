        ...
        <div class="card-body">
            <form action="{{ route('admin.classrooms.update', $classroom->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('room_number') is-invalid @enderror" id="room_number" name="room_number" value="{{ old('room_number', $classroom->room_number) }}">
                    @error('room_number')
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control @error('building_id') is-invalid @enderror" id="building_id" name="building_id">
                        ...
                        @foreach($buildings as $building)
                            <option value="{{ $building->id }}" {{ old('building_id', $classroom->building_id) == $building->id ? 'selected' : '' }}>{{ $building->name }}</option>
                        @endforeach
                    </select>
                    @error('building_id')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control @error('type') is-invalid @enderror" id="type" name="type">
                        <option value="classroom" {{ old('type', $classroom->type) == 'classroom' ? 'selected' : '' }}>Classroom</option>
                        <option value="lab" {{ old('type', $classroom->type) == 'lab' ? 'selected' : '' }}>Lab</option>
                    </select>
                    @error('type')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
